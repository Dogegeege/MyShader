...
//     static Ptr create(const std::string& eventName) { return std::make_shared<EventBase>(eventName); }
...
//     void*       mTarget = nullptr;    /// 某个对象发出的消息的主体
//     void*       mpUserData{nullptr};  /// 附加消息，根据不同的需求，将userData设置成不同的对象指针
// };
...
//         ~Listener() noexcept {
//             if (mFuncionPointerDescriptor.mAddr) { free(mFuncionPointerDescriptor.mAddr); }
//         }
...
//                 /// 按位对比
//                 if (memcmp(mAddr, other.mAddr, mSize) != 0) { return false; }
...
//     template <typename T>
//     auto addEventListener(const std::string& name, T* target, TypedFunctionPointer<T> functionPointer) noexcept -> void;
...
//     /// \param functionPointer
//     template <typename T>
//     auto removeEventListener(const std::string& name, T* target, TypedFunctionPointer<T> functionPointer) noexcept -> void;
...
//     std::mutex                                     mMutex;
//     // Sstatic EventDispatcher*                        mInstance;
// };

// template <typename T>
// auto EventDispatcher::addEventListener(const std::string& name, T* target, TypedFunctionPointer<T> functionPointer) noexcept -> void {
...
//     if (queueIter == mListeners.end()) { queueIter = (mListeners.insert(std::make_pair(name, ListenerQueue()))).first; }
...
//                                      [&](const Listener::Ptr& l) { return EventDispatcher::listenerIsEqual(listener, l); });
//     if (listenerIter != listenerQueue.end()) { return; }

//     listenerQueue.push_back(listener);
// }

// template <typename T>
// auto EventDispatcher::removeEventListener(const std::string& name, T* target, TypedFunctionPointer<T> functionPointer) noexcept -> void {
//     std::lock_guard<std::mutex> lock(mMutex);  /// 加锁

//     auto queueIter = mListeners.find(name);

//     if (queueIter == mListeners.end()) { return; }
...
//     auto listenerIter = std::find_if(listenerQueue.begin(), listenerQueue.end(),
//                                      [&](const Listener::Ptr& l) { return EventDispatcher::listenerIsEqual(listener, l); });
//     if (listenerIter != listenerQueue.end()) { listenerQueue.erase(listenerIter); }
// }
// }  // namespace engine