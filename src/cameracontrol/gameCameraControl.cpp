...
void GameCameraControl::onMouseAction(const MouseAction& action) noexcept {
    if (action == MouseAction::RightDown) {
        ...
        mMouseStateReset = true;  // 标记下次移动的时候，重新设置当前位置
    }
}

void GameCameraControl::onMouseMove(double xpos, double ypos) noexcept {
    if (mRotationState == false) { return; }

    if (mMouseStateReset == true) {
        ...
        /// 不能仰面翻过去，也不能前滚翻
        if (mPitchAngle >= 90.0f) { mPitchAngle = 89.0f; }
...
void GameCameraControl::update() noexcept {
    glm::vec3 front    = mCamera->getLocalDirection();  // 当前 camera 的模型坐标系，负z方向
    ...
    // TODO 使用FPS模式，固定xz轴
    front = mIsUsingFPSMode == true ? front - glm::dot(worldUP, front) * worldUP : front;
    ...
    if (mMoveState[MoveUP]) {
        direction += mIsUsingFPSMode ? worldUP : up;
        needsUpdate = true;
    }
    ...
    if (needsUpdate == true && (direction.x != 0.0 || direction.y != 0.0 || direction.z != 0.0)) {
        // 去除方向向量的长度影响
