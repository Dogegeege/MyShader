...
Attributef::ptr Geometry::getAttribute(const std::string& name) noexcept {
    ...
    if (iter != mAttributes.end()) { return iter->second; }
...
void Geometry::deleteAttribute(const std::string& name) noexcept {
    auto iter = mAttributes.find(name);
    if (iter != mAttributes.end()) { mAttributes.erase(iter); }
}

bool Geometry::hasAttribute(const std::string& name) noexcept {
    auto iter = mAttributes.find(name);
    if (iter == mAttributes.end()) { return false; }
...
void Geometry::computeBoundingBox() noexcept {
    ...
    if (position == nullptr) {
        std::cerr << "Error: geometry has no position when computeBoundingBox" << std::endl;
        return;
    }

    if (mBoundingBox == nullptr) { mBoundingBox = Box3::create(); }
...
//     computeBoundingBox();
//     if (mBoundingSphere == nullptr) { mBoundingSphere = Sphere::create(glm::vec3(0.0f), 0.0f); }
